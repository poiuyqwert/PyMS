...
def _two_leaf_tree() -> SMK.HuffTree:
	writer = _BitWriter()
	...
	return SMK.HuffTree(SMK.BitStream(writer.to_bytes()))


def _build_smk(*, sig: bytes = b'SMK2', width: int = 4, height: int = 4, frames: int = 1, framerate: int = 100, flags: int = 0) -> bytes:
	effective = frames + (1 if flags & SMK.SMK.SMK_FLAG_RING_FRAME else 0)
...
class Test_SMKAudioInfo_load_info(unittest.TestCase):
	def test_defaults(self) -> None:
		info = SMK.SMKAudioInfo()
		info.load_info(0)
		self.assertFalse(info.has_audio)
		self.assertEqual(info.compressed, SMK.SMKAudioInfo.COMPRESSION_NONE)
	...
	def test_has_audio_and_rate(self) -> None:
		info = SMK.SMKAudioInfo()
		info.load_info(SMK.SMKAudioInfo.FLAG_HAS_AUDIO | 22050)
		self.assertTrue(info.has_audio)
		self.assertEqual(info.rate, 22050)

	def test_smk_compression(self) -> None:
		info = SMK.SMKAudioInfo()
		info.load_info(SMK.SMKAudioInfo.FLAG_COMPRESSED)
		self.assertEqual(info.compressed, SMK.SMKAudioInfo.COMPRESSION_SMK)

	def test_bink_compression_overrides_smk(self) -> None:
		info = SMK.SMKAudioInfo()
		info.load_info(SMK.SMKAudioInfo.FLAG_COMPRESSED | SMK.SMKAudioInfo.FLAG_COMPRESS_BINK1)
		self.assertEqual(info.compressed, SMK.SMKAudioInfo.COMPRESSION_BINK)

	def test_bit_depth_and_channels(self) -> None:
		info = SMK.SMKAudioInfo()
		info.load_info(SMK.SMKAudioInfo.FLAG_BIT_DEPTH | SMK.SMKAudioInfo.FLAG_CHANNELS)
...
class Test_SMKFrameInfo_load_info(unittest.TestCase):
	...
	def test_keyframe_and_unknown_flags(self) -> None:
		info = SMK.SMKFrameInfo()
		info.load_info(SMK.SMKFrameInfo.FLAG_KEYFRAME | SMK.SMKFrameInfo.FLAG_UNKNOWN, 0)
	...
	def test_type_passed_through(self) -> None:
		info = SMK.SMKFrameInfo()
		info.load_info(0, SMK.SMKFrameInfo.TYPE_PALETTE)
		self.assertEqual(info.type, SMK.SMKFrameInfo.TYPE_PALETTE)


class Test_BitStream(unittest.TestCase):
	def test_full_byte_lsb_first(self) -> None:
		self.assertEqual(SMK.BitStream(b'\xAB').read(8), 0xAB)

	def test_sequential_reads(self) -> None:
		writer = _BitWriter().write(5, 3).write(200, 8).write(1, 1)
		stream = SMK.BitStream(writer.to_bytes())
	...
	def test_multi_byte_value(self) -> None:
		...
		self.assertEqual(SMK.BitStream(b'\x01\x02').read(16), 0x0201)

	def test_single_bits(self) -> None:
		stream = SMK.BitStream(b'\x01')  # only bit 0 set
		self.assertEqual(stream.read(1), 1)
...
class Test_HuffTree(unittest.TestCase):
	...
	def test_lookup_stops_at_leaf_without_extra_bit(self) -> None:
		...
		tree = _two_leaf_tree()
		stream = SMK.BitStream(bytes([0b1]))  # one bit: navigate to branch1
	...
	def test_absent_tree_is_single_zero_leaf(self) -> None:
		...
		tree = SMK.HuffTree(SMK.BitStream(writer.to_bytes()))
		self.assertIsNone(tree.root.branch0)
		self.assertEqual(tree.root.value, 0)
		self.assertEqual(tree.lookup(SMK.BitStream(b'\x00')), 0)
...
class Test_load_palette(unittest.TestCase):
	def _palette_for(self, data: bytes, current_frame: int = 0, last_palette: list | None = None) -> list:
		...
		smk.frame_info = [SMK.SMKFrameInfo() for _ in range(current_frame + 1)]
		smk.frame_cache = {current_frame: SMK.SMKFrame()}
		if current_frame and last_palette is not None:
			previous = SMK.SMKFrame()
	...
	def test_literal_colors_use_rgb_lookup(self) -> None:
		...
		self.assertEqual(palette[1], (SMK.SMK.RGB_LOOKUP[1], SMK.SMK.RGB_LOOKUP[2], SMK.SMK.RGB_LOOKUP[3]))
	...
	def test_copy_run_without_previous_frame_is_black(self) -> None:
		...
		palette = self._palette_for(data)
		self.assertEqual(len(palette), 256)
		self.assertEqual(palette[0], (SMK.SMK.RGB_LOOKUP[1], SMK.SMK.RGB_LOOKUP[2], SMK.SMK.RGB_LOOKUP[3]))
...
class Test_load(unittest.TestCase):
	def test_smk2(self) -> None:
		...
		self.assertEqual(smk.version, SMK.SMK.SMK2)
	...
	def test_smk4(self) -> None:
		...
		self.assertEqual(smk.version, SMK.SMK.SMK4)
	...
	def test_ring_frame_increments_count(self) -> None:
		smk = SMK.SMK()
		smk.load(_build_smk(frames=1, flags=SMK.SMK.SMK_FLAG_RING_FRAME))
		self.assertEqual(smk.frames, 2)

	def test_invalid_signature_raises(self) -> None:
		with self.assertRaises(PyMSError) as cm:
			SMK.SMK().load(_build_smk(sig=b'XXXX'))
