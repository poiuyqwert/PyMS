...
class Test_get(unittest.TestCase):
	...
	def test_str_enum_decoded(self) -> None:
		self.assertEqual(JSON.get({'c': 'blue'}, 'c', Color), Color.blue)

	def test_str_enum_unknown_case_raises(self) -> None:
		with self.assertRaises(PyMSError) as cm:
			JSON.get({'c': 'green'}, 'c', Color)
		self.assertIn('Invalid JSON format (invalid `c`, invalid enum case)', str(cm.exception))

	def test_str_enum_non_str_value_raises(self) -> None:
		with self.assertRaises(PyMSError) as cm:
			JSON.get({'c': 1}, 'c', Color)
		self.assertIn('Invalid JSON format (invalid `c`)', str(cm.exception))

	def test_int_enum_decoded(self) -> None:
		self.assertEqual(JSON.get({'l': 2}, 'l', Level), Level.high)

	def test_int_enum_unknown_case_raises(self) -> None:
		with self.assertRaises(PyMSError) as cm:
			JSON.get({'l': 3}, 'l', Level)
		self.assertIn('Invalid JSON format (invalid `l`, invalid enum case)', str(cm.exception))

	def test_int_enum_non_int_value_raises(self) -> None:
		with self.assertRaises(PyMSError) as cm:
			JSON.get({'l': 'high'}, 'l', Level)
		self.assertIn('Invalid JSON format (invalid `l`)', str(cm.exception))
...
class Test_get_array(unittest.TestCase):
	...
	def test_str_enum_elements_are_decoded(self) -> None:
		self.assertEqual(JSON.get_array({'cs': ['red', 'blue']}, 'cs', Color), [Color.red, Color.blue])

	def test_str_enum_invalid_element_raises(self) -> None:
		with self.assertRaises(PyMSError) as cm:
			JSON.get_array({'cs': ['red', 'green']}, 'cs', Color)
		self.assertIn('Invalid JSON format (array `cs` contains invalid value at index 1', str(cm.exception))
