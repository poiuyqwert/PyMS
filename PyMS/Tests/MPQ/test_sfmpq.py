...
class Test_SFMPQ_Open(unittest.TestCase):
	def test_open_readonly(self) -> None:
	...
	def test_open_editable(self) -> None:
		mpq = MPQ.SFMPQ(resource_path('test.mpq', __file__))
	...
	def test_open_readonly_open_readonly(self) -> None:
		mpq = MPQ.SFMPQ(resource_path('test.mpq', __file__))
		mpq.open(read_only=True)
		mpq.open(read_only=True)
		mpq.close()

	def test_open_editable_open_readonly(self) -> None:
		mpq = MPQ.SFMPQ(resource_path('test.mpq', __file__))
		mpq.open(read_only=False)
		mpq.open(read_only=True)
		mpq.close()
	...
	def test_with_auto_close(self) -> None:
		mpq = MPQ.SFMPQ(resource_path('test.mpq', __file__))
		mpq.open()
		self.assertTrue(mpq.is_open())
		with mpq.open():
	...
	def test_with_auto_close_override(self) -> None:
		mpq = MPQ.SFMPQ(resource_path('test.mpq', __file__))
		mpq.open()
		self.assertTrue(mpq.is_open())
		with mpq.open():
	...
	def test_used_block_count(self) -> None:
		mpq = MPQ.SFMPQ(resource_path('test.mpq', __file__))
		with mpq.open():
	...
	def test_listfile_open_list_files(self) -> None:
		mpq = MPQ.SFMPQ(resource_path('test.mpq', __file__))
	...
	def test_open_listfile_list_files(self) -> None:
		mpq = MPQ.SFMPQ(resource_path('test.mpq', __file__))
		with mpq.open():
			mpq.add_listfile(resource_path('listfile.txt', __file__))
			files = mpq.list_files()
	...
	def test_has_file_no_locale_has_neutral(self) -> None:
		mpq = MPQ.SFMPQ(resource_path('test.mpq', __file__))
		with mpq.open():
			self.assertTrue(mpq.has_file('test.txt', MPQ.MPQLocale.german))

	def test_has_file_no_locale_no_neutral(self) -> None:
		mpq = MPQ.SFMPQ(resource_path('test.mpq', __file__))
		with mpq.open():
	...
	def test_read_file_no_locale_has_neutral(self) -> None:
		mpq = MPQ.SFMPQ(resource_path('test.mpq', __file__))
		with mpq.open():
...
class Test_SFMPQ_Create(unittest.TestCase):
	...
	def test_create_stay_open(self) -> None:
		mpq = MPQ.SFMPQ(self.path)
		mpq.create()
		self.assertTrue(mpq.is_open())
		self.assertFalse(mpq.is_read_only())
	...
	def test_open_or_create(self) -> None:
		mpq = MPQ.SFMPQ(self.path)
	...
	def test_add_file_locale(self) -> None:
		mpq = MPQ.SFMPQ(self.path)
		with mpq.create():
	...
	def test_add_data_locale(self) -> None:
		mpq = MPQ.SFMPQ(self.path)
		with mpq.create():
	...
	def test_rename_locale(self) -> None:
		mpq = MPQ.SFMPQ(self.path)
		with mpq.create():
			mpq.add_data(b'test', 'test.txt', MPQ.MPQLocale.german)
			mpq.add_data(b'test', 'test.txt', MPQ.MPQLocale.spanish)
			self.assertTrue(mpq.has_file('test.txt', MPQ.MPQLocale.german))
			self.assertTrue(mpq.has_file('test.txt', MPQ.MPQLocale.spanish))
	...
	def test_flush(self) -> None:
		...
		with mpq.create():
			mpq.add_data(b'test', 'test.txt')
			mpq.flush() # Not sure why it needs to be flushed for the block count to update
			self.assertEqual(mpq.used_block_count(), 2) # `(listfile)` and `test.txt`
			mpq.delete_file('test.txt')
			self.assertEqual(mpq.used_block_count(), 2)
			mpq.compact()
