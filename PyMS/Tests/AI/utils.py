...
from ...FileFormats.AIBIN.CodeHandlers import AIParseContext, AIParseSettings, AILexer, DataContext, AISerializeContext
...
def parse_context(code: str, settings: AIParseSettings | None = None, definitions_handler: DefinitionsHandler | None = None, data_context: DataContext | None = None) -> AIParseContext:
	lexer = AILexer(code)
	if settings is None:
		settings = AIParseSettings()
	if definitions_handler is None:
		definitions_handler = DefinitionsHandler()
	if data_context is None:
		data_context = DataContext()
	return AIParseContext(lexer, settings, definitions_handler, data_context)
