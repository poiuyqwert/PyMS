...
class FindReplaceDialog(PyMSDialog):
	...
	def setup_complete(self):
		self.parent.settings.windows.load_window_size('findreplace', self)
	...
	def count(self):
		...
		if f:
			...
			try:
				...
				self.findentry['bg'] = '#FFB4B4'
				return
			MessageBox.showinfo(parent=self, title='Count', message='%s matches found.' % len(r.findall(self.parent.text.get('1.0', END))))
