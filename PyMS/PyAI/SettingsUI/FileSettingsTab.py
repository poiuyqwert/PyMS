...
class FileSettingsTab(SettingsTab):
	def __init__(self, notebook: UI.Notebook, edited_state: EditedState, config: PyAIConfig, mpq_handler: MPQHandler):
		...
		unitsdat.pack(side=UI.TOP, fill=UI.X)
		...
		upgradesdat.pack(side=UI.TOP, fill=UI.X)
		...
		techdat.pack(side=UI.TOP, fill=UI.X)
