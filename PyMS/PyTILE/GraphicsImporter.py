
from __future__ import annotations

from .Config import PyTILEConfig
from .Delegates import GraphicsImporterDelegate, TilePaletteDelegate

from ..FileFormats.Tileset.Tileset import Tileset, TileType, ImportGraphicsOptions
from ..FileFormats.Tileset.VX4 import VX4Minitile

from ..Utilities import UIKit as UI
from ..Utilities.PyMSDialog import PyMSDialog
from ..Utilities.PyMSError import PyMSError
from ..Utilities.ErrorDialog import ErrorDialog
from ..Utilities import Assets
from ..Utilities import Config

class GraphicsImporter(PyMSDialog, TilePaletteDelegate):
	def __init__(self, *, parent: UI.Misc, config: PyTILEConfig, delegate: GraphicsImporterDelegate, tiletype: TileType = TileType.group, ids: list[int] | None = None) -> None:
		self.config_ = config
		self.tiletype = tiletype
		self.ids = ids
		self.delegate = delegate
		PyMSDialog.__init__(self, parent, f'Import {TileType.display_name(tiletype)} Graphics', resizable=(True,True), set_min_size=(True,True))

	def widgetize(self) -> UI.Misc | None:
		tileset = self.delegate.get_tileset()
		assert tileset is not None

		self.replace_selections = UI.BooleanVar()
		self.auto_close = UI.BooleanVar()
		self.megatiles_reuse_duplicates_old = UI.BooleanVar()
		self.megatiles_reuse_duplicates_new = UI.BooleanVar()
		self.megatiles_reuse_null = UI.BooleanVar()
		self.megatiles_null_id = UI.IntegerVar(0,[0,tileset.vf4.megatile_count()-1])
		self.minitiles_reuse_duplicates_old = UI.BooleanVar()
		self.minitiles_reuse_duplicates_new = UI.BooleanVar()
		self.minitiles_reuse_null = UI.BooleanVar()
		self.minitiles_null_id = UI.IntegerVar(0,[0,tileset.vr4.image_count()-1])
		self.load_settings()

		frame = UI.LabelFrame(self, text="BMP's:")
		self.graphics_list = UI.ScrolledListbox(frame, auto_bind=self, selectmode=UI.EXTENDED, height=3)
		self.graphics_list.pack(side=UI.TOP, fill=UI.BOTH, expand=1, padx=2,pady=2)
		self.graphics_list.bind(UI.WidgetEvent.Listbox.Select(), self.update_states)
		buts = UI.Frame(frame)
		button = UI.Button(buts, image=Assets.get_image('find'), width=20, height=20, command=lambda *_: self.select_paths(replace=True))
		button.pack(side=UI.LEFT, padx=(1,0))
		UI.Tooltip(button, "Set BMP's")
		button = UI.Button(buts, image=Assets.get_image('add'), width=20, height=20, command=lambda *_: self.select_paths(replace=False))
		button.pack(side=UI.LEFT)
		UI.Tooltip(button, "Add BMP's")
		button = UI.Button(buts, image=Assets.get_image('remove'), width=20, height=20, command=self.remove_paths)
		button.pack(side=UI.LEFT)
		UI.Tooltip(button, "Remove BMP's")
		self.sel_buttons = [button]
		button = UI.Button(buts, image=Assets.get_image('down'), width=20, height=20, command=self.shift_down)
		button.pack(side=UI.RIGHT, padx=(0,1))
		UI.Tooltip(button, "Move Selections Down")
		self.sel_buttons.append(button)
		button = UI.Button(buts, image=Assets.get_image('up'), width=20, height=20, command=self.shift_up)
		button.pack(side=UI.RIGHT)
		UI.Tooltip(button, "Move Selections Up")
		self.sel_buttons.append(button)
		buts.pack(fill=UI.X)
		frame.pack(side=UI.TOP, fill=UI.BOTH, expand=1, padx=3, pady=2)
		self.settings_frame = UI.LabelFrame(self, text='Settings')
		sets = UI.Frame(self.settings_frame)
		if self.tiletype == TileType.group:
			g = UI.LabelFrame(sets, text='Reuse MegaTiles')
			f = UI.Frame(g)
			UI.Checkbutton(f, text='Duplicates (Old)', variable=self.megatiles_reuse_duplicates_old, anchor=UI.W).pack(side=UI.LEFT)
			f.pack(side=UI.TOP, fill=UI.X)
			f = UI.Frame(g)
			UI.Checkbutton(f, text='Duplicates (New)', variable=self.megatiles_reuse_duplicates_new, anchor=UI.W).pack(side=UI.LEFT)
			f.pack(side=UI.TOP, fill=UI.X)
			f = UI.Frame(g)
			UI.Checkbutton(f, text='Null:', variable=self.megatiles_reuse_null, anchor=UI.W).pack(side=UI.LEFT)
			UI.Entry(f, textvariable=self.megatiles_null_id, font=UI.Font.fixed(), width=5).pack(side=UI.LEFT)
			UI.Button(f, image=Assets.get_image('find'), width=20, height=20, command=lambda *_: self.select_null(TileType.mega)).pack(side=UI.LEFT, padx=1)
			f.pack(side=UI.TOP, fill=UI.X)
			g.grid(column=0,row=0, padx=(0,3), sticky=UI.W+UI.E)
			sets.grid_columnconfigure(0, weight=1)
		g = UI.LabelFrame(sets, text='Reuse MiniTiles')
		f = UI.Frame(g)
		UI.Checkbutton(f, text='Duplicates (Old)', variable=self.minitiles_reuse_duplicates_old, anchor=UI.W).pack(side=UI.LEFT)
		f.pack(side=UI.TOP, fill=UI.X)
		f = UI.Frame(g)
		UI.Checkbutton(f, text='Duplicates (New)', variable=self.minitiles_reuse_duplicates_new, anchor=UI.W).pack(side=UI.LEFT)
		f.pack(side=UI.TOP, fill=UI.X)
		f = UI.Frame(g)
		UI.Checkbutton(f, text='Null:', variable=self.minitiles_reuse_null, anchor=UI.W).pack(side=UI.LEFT)
		UI.Entry(f, textvariable=self.minitiles_null_id, font=UI.Font.fixed(), width=5).pack(side=UI.LEFT)
		UI.Button(f, image=Assets.get_image('find'), width=20, height=20, command=lambda *_: self.select_null(TileType.mini)).pack(side=UI.LEFT, padx=1)
		f.pack(side=UI.TOP, fill=UI.X)
		g.grid(column=1,row=0, sticky=UI.W+UI.E)
		sets.grid_columnconfigure(1, weight=1)
		f = UI.Frame(sets)
		UI.Button(f, text='Reset to recommended settings', command=self.reset_options).pack(side=UI.BOTTOM)
		s = UI.Frame(f)
		UI.Checkbutton(s, text='Replace palette selection', variable=self.replace_selections).pack(side=UI.LEFT)
		UI.Checkbutton(s, text='Auto-close', variable=self.auto_close).pack(side=UI.LEFT, padx=(10,0))
		s.pack(side=UI.BOTTOM)
		f.grid(column=0,row=1, columnspan=2, sticky=UI.W+UI.E)
		sets.pack(fill=UI.X, expand=1, padx=3)
		self.settings_frame.pack(side=UI.TOP, fill=UI.X, padx=3, pady=(3,0))

		buts = UI.Frame(self)
		self.import_button = UI.Button(buts, text='Import', state=UI.DISABLED, command=self.iimport)
		self.import_button.grid(column=0,row=0)
		UI.Button(buts, text='Cancel', command=self.cancel).grid(column=2,row=0)
		buts.grid_columnconfigure(1, weight=1)
		buts.pack(side=UI.BOTTOM, padx=3, pady=3, fill=UI.X)

		self.update_states()

		return self.import_button

	def setup_complete(self) -> None:
		self.window_geometry_config.load_size(self)

	@property
	def window_geometry_config(self) -> Config.WindowGeometry:
		match self.tiletype:
			case TileType.group:
				return self.config_.windows.import_.graphics.group
			case TileType.mega:
				return self.config_.windows.import_.graphics.mega
			case TileType.mini:
				return self.config_.windows.import_.graphics.mini

	@property
	def type_config(self) -> PyTILEConfig.Import.Graphics.MegaSettings | PyTILEConfig.Import.Graphics.MiniSettings:
		match self.tiletype:
			case TileType.group:
				return self.config_.import_.graphics.group
			case TileType.mega:
				return self.config_.import_.graphics.mega
			case TileType.mini:
				return self.config_.import_.graphics.mini

	def load_settings(self) -> None:
		config = self.type_config
		if isinstance(config, PyTILEConfig.Import.Graphics.MegaSettings):
			self.megatiles_reuse_duplicates_old.set(config.megatiles_reuse_duplicates_old.value)
			self.megatiles_reuse_duplicates_new.set(config.megatiles_reuse_duplicates_new.value)
			self.megatiles_reuse_null.set(config.megatiles_reuse_null.value)
			self.megatiles_null_id.set(config.megatiles_null_id.value)
		self.minitiles_reuse_duplicates_old.set(config.minitiles_reuse_duplicates_old.value)
		self.minitiles_reuse_duplicates_new.set(config.minitiles_reuse_duplicates_new.value)
		self.minitiles_reuse_null.set(config.minitiles_reuse_null.value)
		self.minitiles_null_id.set(config.minitiles_null_id.value)
		self.replace_selections.set(config.replace_selections.value)
		self.auto_close.set(config.auto_close.value)

	def update_states(self, _event: UI.Event | None = None) -> None:
		self.import_button['state'] = UI.NORMAL if self.graphics_list.size() else UI.DISABLED
		sel = UI.NORMAL if self.graphics_list.curselection() else UI.DISABLED
		for b in self.sel_buttons:
			b['state'] = sel

	def iimport(self, _event: UI.Event | None = None) -> None:
		tileset = self.delegate.get_tileset()
		if not tileset:
			return
		def can_expand() -> bool:
			return UI.MessageBox.askyesno(parent=self, title='Expand VX4', message="You have run out of minitiles, would you like to expand the VX4 file? If you don't know what this is you should google 'VX4 Expander Plugin' before saying Yes")
		options = ImportGraphicsOptions()
		options.minitiles_reuse_duplicates_old = self.minitiles_reuse_duplicates_old.get()
		options.minitiles_reuse_duplicates_new = self.minitiles_reuse_duplicates_new.get()
		options.minitiles_reuse_null_with_id = self.minitiles_null_id.get() if self.minitiles_reuse_null.get() else None
		options.minitiles_expand_allowed = can_expand
		options.megatiles_reuse_duplicates_old = self.megatiles_reuse_duplicates_old.get()
		options.megatiles_reuse_duplicates_new = self.megatiles_reuse_duplicates_new.get()
		options.megatiles_reuse_null_with_id = self.megatiles_null_id.get() if self.megatiles_reuse_null.get() else None
		ids = None
		if self.replace_selections.get():
			ids = self.ids
		try:
			new_ids = tileset.import_graphics(self.tiletype, self.graphics_list.get(0,UI.END), ids, options)
		except PyMSError as e:
			ErrorDialog(self, e)
		else:
			self.delegate.imported_graphics(new_ids)
			if self.auto_close.get():
				self.ok()

	def select_paths(self, replace: bool = False) -> None:
		paths = self.config_.last_path.graphics.select_open_multiple(self)
		if paths:
			if replace:
				self.graphics_list.delete(0, UI.END)
			self.graphics_list.insert(UI.END, *paths)
			self.graphics_list.xview_moveto(1.0)
			self.graphics_list.yview_moveto(1.0)
		self.update_states()

	def remove_paths(self, _event: UI.Event | None = None) -> None:
		for index in sorted(self.graphics_list.curselection(), reverse=True):
			self.graphics_list.delete(index)
		self.update_states()

	def shift_up(self) -> None:
		min_index = -1
		select = []
		for index in sorted(self.graphics_list.curselection()):
			move_to = index-1
			if move_to > min_index:
				select.append(move_to)
				item = self.graphics_list.get(index)
				self.graphics_list.delete(index)
				self.graphics_list.insert(move_to, item)
			else:
				min_index = index
				select.append(index)
		self.graphics_list.select_clear(0,UI.END)
		for index in select:
			self.graphics_list.select_set(index)

	def shift_down(self) -> None:
		max_index = self.graphics_list.size()
		select = []
		for index in sorted(self.graphics_list.curselection(), reverse=True):
			move_to = index+1
			if move_to < max_index:
				select.append(move_to)
				item = self.graphics_list.get(index)
				self.graphics_list.delete(index)
				self.graphics_list.insert(move_to, item)
			else:
				max_index = index
				select.append(index)
		self.graphics_list.select_clear(0,UI.END)
		for index in select:
			self.graphics_list.select_set(index)

	def select_null(self, tiletype: TileType) -> None:
		tile_id = 0
		if tiletype == TileType.mega:
			tile_id = self.megatiles_null_id.get()
		elif tiletype == TileType.mini:
			tile_id = self.minitiles_null_id.get()
		from .TilePalette import TilePalette  # pylint: disable=cyclic-import
		TilePalette(parent=self, config=self.config_, delegate=self, tiletype=tiletype, select=tile_id)

	def get_tileset(self) -> Tileset | None:
		return self.delegate.get_tileset()

	def get_tile(self, tile_id: int | VX4Minitile) -> UI.Image:
		return self.delegate.get_tile(tile_id)

	def mark_edited(self) -> None:
		return self.delegate.mark_edited()

	def change(self, tiletype: TileType, tile_id: int) -> None:
		if tiletype == TileType.mega:
			self.megatiles_null_id.set(tile_id)
		elif tiletype == TileType.mini:
			self.minitiles_null_id.set(tile_id)

	def megaload(self) -> None:
		pass

	def update_ranges(self) -> None:
		pass

	def reset_options(self) -> None:
		self.type_config.reset()
		self.load_settings()

	def dismiss(self) -> None:
		config = self.type_config
		if isinstance(config, PyTILEConfig.Import.Graphics.MegaSettings):
			config.megatiles_reuse_duplicates_old.value = not not self.megatiles_reuse_duplicates_old.get()
			config.megatiles_reuse_duplicates_new.value = not not self.megatiles_reuse_duplicates_new.get()
			config.megatiles_reuse_null.value = not not self.megatiles_reuse_null.get()
			config.megatiles_null_id.value = self.megatiles_null_id.get()
		config.minitiles_reuse_duplicates_old.value = not not self.minitiles_reuse_duplicates_old.get()
		config.minitiles_reuse_duplicates_new.value = not not self.minitiles_reuse_duplicates_new.get()
		config.minitiles_reuse_null.value = not not self.minitiles_reuse_null.get()
		config.minitiles_null_id.value = self.minitiles_null_id.get()
		config.replace_selections.value = not not self.replace_selections.get()
		config.auto_close.value = not not self.auto_close.get()
		self.window_geometry_config.save_size(self)
		PyMSDialog.dismiss(self)
