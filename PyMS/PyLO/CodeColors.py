...
class CodeColors(PyMSDialog):
	...
	def widgetize(self):
		...
		c.bind(ButtonRelease.Left_Click, lambda e: self.select(0))
		c.grid(sticky=W)
		c = Checkbutton(f, text='Background', variable=self.bg)
		c.bind(ButtonRelease.Left_Click, lambda e: self.select(1))
		c.grid(sticky=W)
		c = Checkbutton(f, text='Bold', variable=self.bold)
		c.bind(ButtonRelease.Left_Click, lambda e: self.select(2))
		c.grid(sticky=W)
		self.fgcanvas = Canvas(f, width=32, height=32, background='#000000')
		self.fgcanvas.bind(Mouse.Click, lambda e: self.colorselect(0))
		self.fgcanvas.grid(column=1, row=0)
		self.bgcanvas = Canvas(f, width=32, height=32, background='#000000')
		self.bgcanvas.bind(Mouse.Click, lambda e: self.colorselect(1))
