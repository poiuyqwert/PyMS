
from __future__ import annotations

from .DATTab import DATTab
from ..DataID import DATID, DataID, UnitsTabID, AnyID
from ..DATRef import DATRefs, DATRef
from ..IconSelectDialog import IconSelectDialog

from ...FileFormats.DAT import DATWeapon, DATUnit, DATOrder

from ...Utilities import UIKit as UI
from ...Utilities import Assets

from typing import TYPE_CHECKING, cast
if TYPE_CHECKING:
	from ..Delegates import MainDelegate

class WeaponsTab(DATTab):
	DAT_ID = DATID.weapons

	def __init__(self, parent: UI.Misc, delegate: MainDelegate) -> None:
		DATTab.__init__(self, parent, delegate)
		scrollview = UI.ScrollView(self)

		self.amount = UI.IntegerVar(0, [0,65535])
		self.type = UI.IntVar()
		self.bonus = UI.IntegerVar(0, [0,65535])
		self.explosion = UI.IntVar()
		self.factor = UI.IntegerVar(0, [0,255])
		self.unused = UI.IntVar()
		self.cooldown = UI.IntegerVar(0, [1,255])
		self.seconds = UI.FloatVar(1, [1/24.0,255/24.0], callback=lambda time: self.update_ticks(time, self.cooldown), precision=2)
		self.cooldown.callback = lambda ticks: self.update_time(ticks, self.seconds)
		self.upgradeentry = UI.IntegerVar(0,[0,61])
		self.upgrade = UI.IntVar()

		data = UI.Frame(scrollview.content_view)
		left = UI.Frame(data)
		l = UI.LabelFrame(left, text='Damage Properties:')
		s = UI.Frame(l)
		f = UI.Frame(s)
		ls = UI.Frame(f)
		UI.Label(ls, text='Amount:', width=12, anchor=UI.E).pack(side=UI.LEFT)
		UI.Entry(ls, textvariable=self.amount, font=UI.Font.fixed(), width=5).pack(side=UI.LEFT, padx=2)
		self.tip(ls, 'Damage Amount', 'WeapDamageAmount')
		ls.pack(side=UI.LEFT)
		ls = UI.Frame(f)
		UI.Label(ls, text='Type:', width=9, anchor=UI.E).pack(side=UI.LEFT)
		UI.DropDown(ls, self.type, Assets.data_cache(Assets.DataReference.DamTypes), width=20).pack(side=UI.LEFT, fill=UI.X, expand=1, padx=2)
		self.tip(ls, 'Damage Type', 'WeapDamageType')
		ls.pack(side=UI.LEFT)
		f.pack(fill=UI.X)
		f = UI.Frame(s)
		ls = UI.Frame(f)
		UI.Label(ls, text='Bonus:', width=12, anchor=UI.E).pack(side=UI.LEFT)
		UI.Entry(ls, textvariable=self.bonus, font=UI.Font.fixed(), width=5).pack(side=UI.LEFT, padx=2)
		self.tip(ls, 'Damage Bonus', 'WeapDamageBonus')
		ls.pack(side=UI.LEFT)
		ls = UI.Frame(f)
		UI.Label(ls, text='Explosion:', width=9, anchor=UI.E).pack(side=UI.LEFT)
		UI.DropDown(ls, self.explosion, Assets.data_cache(Assets.DataReference.Explosions), width=20).pack(side=UI.LEFT, fill=UI.X, expand=1, padx=2)
		self.tip(ls, 'Explosion', 'WeapDamageExplosion')
		ls.pack(side=UI.LEFT)
		f.pack(fill=UI.X)
		f = UI.Frame(s)
		ls = UI.Frame(f)
		UI.Label(ls, text='Factor:', width=12, anchor=UI.E).pack(side=UI.LEFT)
		UI.Entry(ls, textvariable=self.factor, font=UI.Font.fixed(), width=5).pack(side=UI.LEFT, padx=2)
		self.tip(ls, 'Damage Factor', 'WeapDamageFactor')
		ls.pack(side=UI.LEFT)
		ls = UI.Frame(f)
		UI.Label(ls, text='Unused:', width=9, anchor=UI.E).pack(side=UI.LEFT)
		self.unused_ddw = UI.DropDown(ls, self.unused, [], width=20)
		self.unused_ddw.pack(side=UI.LEFT, fill=UI.X, expand=1, padx=3)
		self.tip(ls, 'Unused Technology', 'WeapUnused')
		ls.pack(side=UI.LEFT)
		f.pack(fill=UI.X)
		f = UI.Frame(s)
		UI.Label(f, text='Cooldown:', width=12, anchor=UI.E).pack(side=UI.LEFT)
		UI.Entry(f, textvariable=self.cooldown, font=UI.Font.fixed(), width=5).pack(side=UI.LEFT, padx=2)
		UI.Label(f, text='=').pack(side=UI.LEFT)
		UI.Entry(f, textvariable=self.seconds, font=UI.Font.fixed(), width=7).pack(side=UI.LEFT, padx=2)
		UI.Label(f, text='secs.').pack(side=UI.LEFT)
		self.tip(f, 'Cooldown', 'WeapDamageCooldown')
		f.pack(fill=UI.X)
		f = UI.Frame(s)
		UI.Label(f, text='Upgrade:', width=12, anchor=UI.E).pack(side=UI.LEFT)
		UI.Entry(f, textvariable=self.upgradeentry, font=UI.Font.fixed(), width=2).pack(side=UI.LEFT, padx=2)
		UI.Label(f, text='=').pack(side=UI.LEFT)
		self.upgrade_ddw = UI.DropDown(f, self.upgrade, [], self.upgradeentry, width=18)
		self.upgrade_ddw.pack(side=UI.LEFT, fill=UI.X, expand=1, padx=2)
		UI.Button(f, text='Jump ->', command=lambda: self.jump(DATID.upgrades, self.upgrade.get())).pack(side=UI.LEFT)
		self.tip(f, 'Damage Upgrade', 'WeapDamageUpgrade')
		f.pack(fill=UI.X)
		s.pack(fill=UI.BOTH, padx=5, pady=5)
		l.pack(fill=UI.X)

		self.labelentry = UI.IntegerVar(0,[0,0])
		self.label = UI.IntVar()
		self.errormsgentry = UI.IntegerVar(0,[0,0])
		self.errormsg = UI.IntVar()

		l = UI.LabelFrame(left, text='Weapon Display:')
		s = UI.Frame(l)
		f = UI.Frame(s)
		UI.Label(f, text='Label:', width=12, anchor=UI.E).pack(side=UI.LEFT)
		UI.Entry(f, textvariable=self.labelentry, font=UI.Font.fixed(), width=5).pack(side=UI.LEFT)
		UI.Label(f, text='=').pack(side=UI.LEFT)
		self.labels = UI.DropDown(f, self.label, [], self.labelentry, width=28)
		self.labels.pack(side=UI.LEFT, fill=UI.X, expand=1, padx=2)
		self.tip(f, 'Label', 'WeapLabel')
		f.pack(fill=UI.X)
		f = UI.Frame(s)
		UI.Label(f, text='Error Msg:', width=12, anchor=UI.E).pack(side=UI.LEFT)
		UI.Entry(f, textvariable=self.errormsgentry, font=UI.Font.fixed(), width=5).pack(side=UI.LEFT)
		UI.Label(f, text='=').pack(side=UI.LEFT)
		self.errormsgs = UI.DropDown(f, self.errormsg, [], self.errormsgentry, width=28)
		self.errormsgs.pack(side=UI.LEFT, fill=UI.X, expand=1, padx=2)
		self.tip(f, 'Error Message', 'WeapError')
		f.pack(fill=UI.X)
		s.pack(fill=UI.BOTH, padx=5, pady=5)
		l.pack(fill=UI.X)

		self.removeafter = UI.IntegerVar(0, [0,255])
		self.behaviour = UI.IntVar()
		self.graphicsentry = UI.IntegerVar(0, [0,208])
		self.graphicsdd = UI.IntVar()
		self.iconentry = UI.IntegerVar(0, [0,389], callback=lambda n:self.selicon(n,1))
		self.icondd = UI.IntVar()
		self.xoffset = UI.IntegerVar(0, [0,255])
		self.yoffset = UI.IntegerVar(0, [0,255])
		self.attackangle = UI.IntegerVar(0, [0,255])
		self.launchspin = UI.IntegerVar(0, [0,255])

		l = UI.LabelFrame(left, text='Weapon Display:')
		s = UI.Frame(l)
		f = UI.Frame(s)
		UI.Label(f, text='Behaviour:', width=12, anchor=UI.E).pack(side=UI.LEFT)
		UI.DropDown(f, self.behaviour, Assets.data_cache(Assets.DataReference.Behaviours), width=30).pack(side=UI.LEFT, fill=UI.X, expand=1, padx=2)
		self.tip(f, 'Behaviour', 'WeapBehaviour')
		f.pack(fill=UI.X)
		f = UI.Frame(s)
		UI.Label(f, text='Remove After:', width=12, anchor=UI.E).pack(side=UI.LEFT)
		UI.Entry(f, textvariable=self.removeafter, font=UI.Font.fixed(), width=5).pack(side=UI.LEFT, padx=2)
		self.tip(f, 'Remove After', 'WeapRemoveAfter')
		f.pack(fill=UI.X)
		f = UI.Frame(s)
		UI.Label(f, text='Graphics:', width=12, anchor=UI.E).pack(side=UI.LEFT)
		UI.Entry(f, textvariable=self.graphicsentry, font=UI.Font.fixed(), width=5).pack(side=UI.LEFT, padx=2)
		UI.Label(f, text='=').pack(side=UI.LEFT)
		self.graphics_ddw = UI.DropDown(f, self.graphicsdd, [], self.graphicsentry)
		self.graphics_ddw.pack(side=UI.LEFT, fill=UI.X, expand=1, padx=2)
		UI.Button(f, text='Jump ->', command=lambda: self.jump(DATID.flingy, self.graphicsdd.get())).pack(side=UI.LEFT, padx=2)
		self.tip(f, 'Graphics', 'WeapGraphics')
		f.pack(fill=UI.X)
		f = UI.Frame(s)
		UI.Label(f, text='Icon:', width=12, anchor=UI.E).pack(side=UI.LEFT)
		UI.Entry(f, textvariable=self.iconentry, font=UI.Font.fixed(), width=5).pack(side=UI.LEFT, padx=2)
		UI.Label(f, text='=').pack(side=UI.LEFT)
		self.icon_ddw = UI.DropDown(f, self.icondd, [], self.selicon)
		self.icon_ddw.pack(side=UI.LEFT, fill=UI.X, expand=1, padx=2)
		self.tip(f, 'Icon', 'WeapIcon')
		UI.Button(f, image=Assets.get_image('find'), command=self.choose_icon, width=20, height=20).pack(side=UI.LEFT, padx=2)
		f.pack(fill=UI.X)
		f = UI.Frame(s)
		ls = UI.Frame(f)
		rs = UI.Frame(ls)
		UI.Label(rs, text='X Offset:', width=12, anchor=UI.E).pack(side=UI.LEFT)
		UI.Entry(rs, textvariable=self.xoffset, font=UI.Font.fixed(), width=5).pack(side=UI.LEFT, padx=2)
		self.tip(rs, 'X Offset', 'WeapOffsetForward')
		rs.pack(side=UI.TOP)
		rs = UI.Frame(ls)
		UI.Label(rs, text='Y Offset:', width=12, anchor=UI.E).pack(side=UI.LEFT)
		UI.Entry(rs, textvariable=self.yoffset, font=UI.Font.fixed(), width=5).pack(side=UI.LEFT, padx=2)
		self.tip(rs, 'Y Offset', 'WeapOffsetUpward')
		rs.pack(side=UI.TOP)
		ls.pack(side=UI.LEFT)
		ls = UI.Frame(f)
		rs = UI.Frame(ls)
		UI.Label(rs, text='Attack Angle:', width=12, anchor=UI.E).pack(side=UI.LEFT)
		UI.Entry(rs, textvariable=self.attackangle, font=UI.Font.fixed(), width=3).pack(side=UI.LEFT, padx=2)
		self.tip(rs, 'Attack Angle', 'WeapAttackAngle')
		rs.pack(side=UI.TOP)
		rs = UI.Frame(ls)
		UI.Label(rs, text='Launch Spin:', width=12, anchor=UI.E).pack(side=UI.LEFT)
		UI.Entry(rs, textvariable=self.launchspin, font=UI.Font.fixed(), width=3).pack(side=UI.LEFT, padx=2)
		self.tip(rs, 'Launch Spin', 'WeapLaunchSpin')
		rs.pack(side=UI.TOP)
		ls.pack(side=UI.LEFT)
		ls = UI.Frame(f, relief=UI.SUNKEN, bd=1)
		self.preview = UI.Canvas(ls, width=34, height=34, background='#000000', theme_tag='preview') # type: ignore[call-arg]
		self.preview.pack()
		self.preview.bind(UI.Mouse.Click_Left(), lambda *_: self.choose_icon())
		ls.pack(side=UI.RIGHT)
		f.pack(fill=UI.X)
		f = UI.Frame(s)
		f.pack(fill=UI.X)
		s.pack(fill=UI.BOTH, padx=5, pady=5)
		l.pack(fill=UI.X)
		left.pack(side=UI.LEFT, fill=UI.Y)

		self.minrange = UI.IntegerVar(0, [0,4294967294])
		self.maxrange = UI.IntegerVar(0, [0,4294967294])

		right = UI.Frame(data)
		l = UI.LabelFrame(right, text='Weapon Range:')
		s = UI.Frame(l)
		f = UI.Frame(s)
		UI.Label(f, text='Min:', width=12, anchor=UI.E).pack(side=UI.LEFT)
		UI.Entry(f, textvariable=self.minrange, font=UI.Font.fixed(), width=10).pack(side=UI.LEFT, padx=2)
		self.tip(f, 'Minimum Range', 'WeapRangeMin')
		f.pack(fill=UI.X)
		f = UI.Frame(s)
		UI.Label(f, text='Max:', width=12, anchor=UI.E).pack(side=UI.LEFT)
		UI.Entry(f, textvariable=self.maxrange, font=UI.Font.fixed(), width=10).pack(side=UI.LEFT, padx=2)
		self.tip(f, 'Maximum Range', 'WeapRangeMax')
		f.pack(fill=UI.X)
		s.pack(fill=UI.BOTH, padx=5, pady=5)
		l.pack()

		self.innerradius = UI.IntegerVar(0, [0,65535])
		self.mediumradius = UI.IntegerVar(0, [0,65535])
		self.outerradius = UI.IntegerVar(0, [0,65535])

		l = UI.LabelFrame(right, text='Splash Radii:')
		s = UI.Frame(l)
		f = UI.Frame(s)
		UI.Label(f, text='Inner:', width=12, anchor=UI.E).pack(side=UI.LEFT)
		UI.Entry(f, textvariable=self.innerradius, font=UI.Font.fixed(), width=5).pack(side=UI.LEFT, padx=2)
		self.tip(f, 'Inner Radius', 'WeapSplashInner')
		f.pack(fill=UI.X)
		f = UI.Frame(s)
		UI.Label(f, text='Medium:', width=12, anchor=UI.E).pack(side=UI.LEFT)
		UI.Entry(f, textvariable=self.mediumradius, font=UI.Font.fixed(), width=5).pack(side=UI.LEFT, padx=2)
		self.tip(f, 'Medium Radius', 'WeapSplashMed')
		f.pack(fill=UI.X)
		f = UI.Frame(s)
		UI.Label(f, text='Outer:', width=12, anchor=UI.E).pack(side=UI.LEFT)
		UI.Entry(f, textvariable=self.outerradius, font=UI.Font.fixed(), width=5).pack(side=UI.LEFT, padx=2)
		self.tip(f, 'Outer Radius', 'WeapSplashOuter')
		f.pack(fill=UI.X)
		s.pack(fill=UI.BOTH, padx=5, pady=5)
		l.pack(fill=UI.X)

		self.air = UI.IntVar()
		self.ground = UI.IntVar()
		self.mechanical = UI.IntVar()
		self.organic = UI.IntVar()
		self.nonbuilding = UI.IntVar()
		self.nonrobotic = UI.IntVar()
		self.terrain = UI.IntVar()
		self.orgormech = UI.IntVar()
		self.own = UI.IntVar()

		l = UI.LabelFrame(right, text='Target Flags:')
		s = UI.Frame(l)
		flags = [
			('Air', self.air, '001'),
			('Ground', self.ground, '002'),
			('Mechanical', self.mechanical, '004'),
			('Organic', self.organic, '008'),
			('Non-Building', self.nonbuilding, '010'),
			('Non-Robotic', self.nonrobotic, '020'),
			('Terrain', self.terrain, '040'),
			('Org. or Mech.', self.orgormech, '080'),
			('Own', self.own, '100'),
		]
		for t,v,h in flags:
			f = UI.Frame(s)
			UI.Checkbutton(f, text=t, variable=v).pack(side=UI.LEFT)
			self.tip(f, t, 'WeapTarget' + h)
			f.pack(side=UI.TOP, fill=UI.X)
		s.pack(fill=UI.BOTH, padx=5, pady=5)
		l.pack(fill=UI.BOTH)
		right.pack(side=UI.LEFT)
		data.pack(pady=5)
		scrollview.pack(fill=UI.BOTH, expand=1)

		self.setup_used_by((
			DATRefs(DATID.units, lambda unit: (
				DATRef('Ground Weapon', cast(DATUnit, unit).ground_weapon, dat_sub_tab=UnitsTabID.basic),
				DATRef('Air Weapon', cast(DATUnit, unit).air_weapon, dat_sub_tab=UnitsTabID.basic)
			)),
			DATRefs(DATID.orders, lambda order: (
				DATRef('Targeting', cast(DATOrder, order).weapon_targeting),
			)),
		))

	def updated_pointer_entries(self, ids: list[AnyID]) -> None:
		if DataID.stat_txt in ids:
			strings = ('None',) + self.delegate.data_context.stat_txt.strings
			self.labels.setentries(strings)
			self.errormsgs.setentries(strings)
		if DataID.cmdicons in ids:
			self.icon_ddw.setentries(self.delegate.data_context.cmdicons.names)

		if (DATID.units in ids or DATID.orders in ids) and self.delegate.active_tab() == self:
			self.check_used_by_references()
		if DATID.techdata in ids:
			self.unused_ddw.setentries(self.delegate.data_context.technology.names + ('None',))
		if DATID.upgrades in ids:
			self.upgrade_ddw.setentries(self.delegate.data_context.upgrades.names + ('None',))
		if DATID.flingy in ids:
			self.graphics_ddw.setentries(self.delegate.data_context.flingy.names)

		if self.delegate.data_context.config.settings.reference_limits.value:
			if DATID.upgrades in ids:
				self.upgradeentry.range[1] = self.delegate.data_context.upgrades.entry_count() - 1
			if DATID.flingy in ids:
				self.graphicsentry.range[1] = self.delegate.data_context.flingy.entry_count() - 1
		else:
			self.upgradeentry.range[1] = 255
			self.graphicsentry.range[1] = 4294967295
		string_limit = len(self.delegate.data_context.stat_txt.strings) - 1
		self.labelentry.range[1] = string_limit
		self.errormsgentry.range[1] = string_limit
		self.iconentry.range[1] = self.delegate.data_context.cmdicons.frame_count() - 1

	def selicon(self, n: int, t: int = 0) -> None:
		if t:
			self.icondd.set(n)
		else:
			self.iconentry.set(n)
		self.drawpreview()

	def choose_icon(self) -> None:
		def update_icon(index: int) -> None:
			self.iconentry.set(index)
		IconSelectDialog(self, data_context=self.delegate.data_context, delegate=update_icon, selected_index=self.iconentry.get())

	def drawpreview(self) -> None:
		self.preview.delete(UI.ALL)
		index = self.iconentry.get()
		image = self.delegate.data_context.get_cmdicon(index)
		if image:
			self.preview.create_image(19-image[1]//2+(image[0].width()-image[2])//2, 19-image[3]//2+(image[0].height()-image[4])//2, image=image[0])

	def load_entry(self, entry: DATWeapon) -> None:
		self.label.set(entry.label)
		self.graphicsentry.set(entry.graphics)
		self.unused.set(entry.unused_technology)

		target_flags_fields = (
			(self.air, DATWeapon.TargetFlag.air),
			(self.ground, DATWeapon.TargetFlag.ground),
			(self.mechanical, DATWeapon.TargetFlag.mechanical),
			(self.organic, DATWeapon.TargetFlag.organic),
			(self.nonbuilding, DATWeapon.TargetFlag.non_building),
			(self.nonrobotic, DATWeapon.TargetFlag.non_robotic),
			(self.terrain, DATWeapon.TargetFlag.terrain),
			(self.orgormech, DATWeapon.TargetFlag.organic_or_mechanical),
			(self.own, DATWeapon.TargetFlag.own )
		)
		for (variable, flag) in target_flags_fields:
			variable.set(entry.target_flags & flag == flag)

		self.minrange.set(entry.minimum_range)
		self.maxrange.set(entry.maximum_range)
		self.upgrade.set(entry.damage_upgrade)
		self.type.set(entry.weapon_type)
		self.behaviour.set(entry.weapon_behavior)
		self.removeafter.set(entry.remove_after)
		self.explosion.set(entry.explosion_type)
		self.innerradius.set(entry.inner_splash_range)
		self.mediumradius.set(entry.medium_splash_range)
		self.outerradius.set(entry.outer_splash_range)
		self.amount.set(entry.damage_amount)
		self.bonus.set(entry.damage_bonus)
		self.cooldown.set(entry.weapon_cooldown)
		self.factor.set(entry.damage_factor)
		self.attackangle.set(entry.attack_angle)
		self.launchspin.set(entry.launch_spin)
		self.xoffset.set(entry.forward_offset)
		self.yoffset.set(entry.upward_offset)
		self.errormsg.set(entry.target_error_message)
		self.iconentry.set(entry.icon)

		self.drawpreview()

	def save_entry(self, entry: DATWeapon) -> None:
		if self.label.get() != entry.label:
			entry.label = self.label.get()
			self.edited = True
			if self.delegate.data_context.config.settings.labels.custom.value:
				self.delegate.data_context.dat_data(DATID.weapons).update_names()
		if self.graphicsentry.get() != entry.graphics:
			entry.graphics = self.graphicsentry.get()
			self.edited = True
		if self.unused.get() != entry.unused_technology:
			entry.unused_technology = self.unused.get()
			self.edited = True

		target_flags = entry.target_flags & ~DATWeapon.TargetFlag.ALL_FLAGS
		target_flags_fields = (
			(self.air, DATWeapon.TargetFlag.air),
			(self.ground, DATWeapon.TargetFlag.ground),
			(self.mechanical, DATWeapon.TargetFlag.mechanical),
			(self.organic, DATWeapon.TargetFlag.organic),
			(self.nonbuilding, DATWeapon.TargetFlag.non_building),
			(self.nonrobotic, DATWeapon.TargetFlag.non_robotic),
			(self.terrain, DATWeapon.TargetFlag.terrain),
			(self.orgormech, DATWeapon.TargetFlag.organic_or_mechanical),
			(self.own, DATWeapon.TargetFlag.own )
		)
		for (variable, flag) in target_flags_fields:
			if variable.get():
				target_flags |= flag
		if target_flags != entry.target_flags:
			entry.target_flags = target_flags
			self.edited = True

		if self.minrange.get() != entry.minimum_range:
			entry.minimum_range = self.minrange.get()
			self.edited = True
		if self.maxrange.get() != entry.maximum_range:
			entry.maximum_range = self.maxrange.get()
			self.edited = True
		if self.upgrade.get() != entry.damage_upgrade:
			entry.damage_upgrade = self.upgrade.get()
			self.edited = True
		if self.type.get() != entry.weapon_type:
			entry.weapon_type = self.type.get()
			self.edited = True
		if self.behaviour.get() != entry.weapon_behavior:
			entry.weapon_behavior = self.behaviour.get()
			self.edited = True
		if self.removeafter.get() != entry.remove_after:
			entry.remove_after = self.removeafter.get()
			self.edited = True
		if self.explosion.get() != entry.explosion_type:
			entry.explosion_type = self.explosion.get()
			self.edited = True
		if self.innerradius.get() != entry.inner_splash_range:
			entry.inner_splash_range = self.innerradius.get()
			self.edited = True
		if self.mediumradius.get() != entry.medium_splash_range:
			entry.medium_splash_range = self.mediumradius.get()
			self.edited = True
		if self.outerradius.get() != entry.outer_splash_range:
			entry.outer_splash_range = self.outerradius.get()
			self.edited = True
		if self.amount.get() != entry.damage_amount:
			entry.damage_amount = self.amount.get()
			self.edited = True
		if self.bonus.get() != entry.damage_bonus:
			entry.damage_bonus = self.bonus.get()
			self.edited = True
		if self.cooldown.get() != entry.weapon_cooldown:
			entry.weapon_cooldown = self.cooldown.get()
			self.edited = True
		if self.factor.get() != entry.damage_factor:
			entry.damage_factor = self.factor.get()
			self.edited = True
		if self.attackangle.get() != entry.attack_angle:
			entry.attack_angle = self.attackangle.get()
			self.edited = True
		if self.launchspin.get() != entry.launch_spin:
			entry.launch_spin = self.launchspin.get()
			self.edited = True
		if self.xoffset.get() != entry.forward_offset:
			entry.forward_offset = self.xoffset.get()
			self.edited = True
		if self.yoffset.get() != entry.upward_offset:
			entry.upward_offset = self.yoffset.get()
			self.edited = True
		if self.errormsg.get() != entry.target_error_message:
			entry.target_error_message = self.errormsg.get()
			self.edited = True
		if self.iconentry.get() != entry.icon:
			entry.icon = self.iconentry.get()
			self.edited = True
