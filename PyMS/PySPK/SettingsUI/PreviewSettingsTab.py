...
class PreviewSettingsTab(SettingsTab):
	def __init__(self, notebook: UI.Notebook, edited_state: EditedState, config: PySPKConfig, mpq_handler: MPQHandler):
		...
		platform_pal = FileSettingView(parent=self, edited_state=edited_state, name='platform.wpe', description='The palette which holds the star palette.', setting=config.settings.files.platform_wpe, mpq_handler=mpq_handler, mpq_history_config=config.settings.mpq_select_history, mpq_window_geometry_config=config.windows.settings.mpq_select)
		platform_pal.pack(side=UI.TOP, fill=UI.X)
		self.register_settings_view(platform_pal)
