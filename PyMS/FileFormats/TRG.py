...
def condition_comparison(trg, decompile, condition, data=None):
	...
	if decompile:
		if not condition[4]:
			...
			return 'Exactly'
		raise PyMSError('Decompile', "Invalid Comparison '%s' (value most be one of: 0, 1, 10)" % condition[4])
...
def condition_set(trg, decompile, condition, data=None):
	...
	if decompile:
		if condition[4] == 2:
			...
			return 'Cleared'
		raise PyMSError('Decompile', "Invalid Set '%s' (value most be one of: 2, 3)" % condition[4])
...
def action_modifier(trg, decompile, action, strings, properties, data=None):
	...
	if decompile:
		if action[8] == 7:
			...
			return 'Subtract'
		raise PyMSError('Decompile', "Invalid Modifier '%s' (value most be one of: 7, 8, 9)" % action[8])
...
def action_switchaction(trg, decompile, action, strings, properties, data=None):
	...
	if decompile:
		if action[8] == 4:
			...
			return 'Randomize'
		raise PyMSError('Decompile', "Invalid SwitchAction '%s' (value most be one of: 4, 5, 6, 11)" % action[8])
...
def action_state(trg, decompile, action, strings, properties, data=None):
	...
	if decompile:
		if action[8] == 4:
			...
			return 'Toggle'
		raise PyMSError('Decompile', "Invalid State '%s' (value most be one of: 4, 5, 6)" % action[8])
...
def action_percentage(trg, decompile, action, strings, properties, data=None):
	...
	if decompile:
		if action[5] > 100:
			raise PyMSError('Decompile', "Invalid Percentage '%s' (value most be in the range 0 to 100)" % action[5])
		return '%s%%' % action[2]
	try:
		if data.endswith('%'):
			p = int(data[:-1])
...
def action_slot(trg, decompile, action, strings, properties, data=None):
	"""Slot     - A number from 1 to 4, or a keyword in the list: Slot1, Slot2, Slot3, Slot4"""
	if decompile:
		return 'Slot%d' % (action[4] + 1)
	slot = 0
	if data.startswith('Slot'):
		slot = int(data[4:].strip())-1
	else:
		slot = int(data)-1
	if slot < 0 or slot > 3:
		raise PyMSError('Parameter',"'%s' is an invalid Slot (value must be in the range 1 to 4, or a keyword in the list: Slot1, Slot2, Slot3, Slot4)" % data)
	action[4] = slot
...
def new_math(trg, decompile, action, strings, properties, data=None):
	...
	if decompile:
		if action[8] == 7:
			...
			return 'Divide'
		raise PyMSError('Decompile', "Invalid Math operator '%s' (value most be one of: 7, 8, 9, 12, 13)" % action[8])
...
class TRG:
	...
	action_parameters = {
		...
		MISSION_BRIEFING:[
			...
			[action_time],
			[action_wav, action_time],
			[action_string, action_time],
			[action_string],
			[action_unit, action_slot],
			[action_slot],
			[action_slot, action_time],
			[action_string, action_slot, action_time, action_modifier, action_wav, action_time],
			None,
  ...
  # // Modifier = Set, Add, Subtract
	...
	def load_data(self, data, TRIG=False, MBRF=False):
		try:
				...
				if not findnext:
						...
						# elif condition[5] == 22 and condition[2] != 0:
							# if self.dynamic_conditions[condition[2]][1]:
								# for parameter in self.dynamic_conditions[condition[2]][1]:
									# parameter(self, True, condition, {}, {})
						# else:
							# if self.condition_parameters[triggers_type][condition[5]]:
								# for parameter in self.condition_parameters[triggers_type][condition[5]]:
									# parameter(self, True, condition)
					...
					for a in range(64):
						...
						# else:
							...
							# if self.action_parameters[triggers_type][action[7]]:
								# for parameter in self.action_parameters[triggers_type][action[7]]:
									# parameter(self, True, action, {}, {})
						if not TRIG:
							if action[1]:
								findnext.append([True,action[1]])
			...
			# for t in triggers:
				# d.write(str(t) + '\n')
			# d.close()
			self.triggers_type = triggers_type
	...
	def interpret(self, file): 
		...
		def include(ddata, triggers_type=None):
			id = 0
			state = 0
								...
								if match:
									...
									if cmd in conditions:
										...
										if cmd in self.conditions[triggers_type]:
											condition[5] = self.conditions[triggers_type].index(cmd)
											params = self.condition_parameters[triggers_type][condition[5]]
								...
								if match:
									...
									if cmd in actions:
										...
										if cmd in self.actions[triggers_type]:
											action[7] = self.actions[triggers_type].index(cmd)
											params = self.action_parameters[triggers_type][action[7]]
	...
	def reference(self, MBRF=False):
		result = '#----------------------------------------------------\n# Parameter Types:\n'
		done = []
		triggers_type = MISSION_BRIEFING if MBRF else NORMAL_TRIGGERS
		for p in self.condition_parameters[triggers_type] + self.action_parameters[triggers_type]:
			if p:
				for t in p:
					if t:
		...
		result += '#\n# Conditions:\n'
		for c,ps in zip(self.conditions[triggers_type],self.condition_parameters[triggers_type]):
			if c:
				result += '#    %s(' % c
				if ps:
					comma = False
					for p in ps:
						if comma:
							...
							comma = True
						result += p.__doc__.split(' ',1)[0]
				result += ')\n'
		result += '#\n# Actions:\n'
		for c,ps in zip(self.actions[triggers_type],self.action_parameters[triggers_type]):
			result += '#    %s(' % c
			if ps:
				comma = False
				for p in ps:
	...
	def decompile_data(self, ref=False):
		result = ''
		if self.triggers_type == MISSION_BRIEFING:
				...
				if params:
					...
					for parameter in params:
						if comma:
							...
							comma = True
						result += str(parameter(self, True, condition))
				result += ')\n'
			result += '\tActions:\n'
				...
				if params:
					...
					for parameter in params:
						if comma:
							...
							comma = True
						result += str(parameter(self, True, action, self.strings, self.properties))
				result += ')\n'
			result += '\n'
		return result
	...
	def compile_data(self, TRIG=False):
		result = ''
		if not TRIG:
			result += 'qw\x986\x18\x00\x00\x00'
		props = []
		for trigger in self.triggers:
			writenext = []
			for condition in trigger[1]:
				result += struct.pack('<3LH4Bxx', *condition)
			result += '\x00' * 20 * (16 - len(trigger[1]))
			for action in trigger[2]:
				if not action[7] in [58,59]:
					if not TRIG:
						...
						action[9] ^= 2
				result += struct.pack('<6LH3B3x', *action)
			result += '\x00' * 32 * (64 - len(trigger[2]))
			result += struct.pack('4x28B', *trigger[0])
			for type,id in writenext:
				if type:
					result += self.strings[id] + '\x00' * (2048 - len(self.strings[id]))
				else:
					result += struct.pack('<HH4BLHH4x', *self.properties[id])
		return result

for triggers_type in [NORMAL_TRIGGERS,MISSION_BRIEFING]:
	for params in TRG.condition_parameters[triggers_type] + TRG.action_parameters[triggers_type] + TRG.new_action_parameters:
		if params:
			for p in params:
				if p.__doc__:
					d = p.__doc__.split('-',1)
