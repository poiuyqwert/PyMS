...
class AbstractDAT:
	...
	@overload
	def export_entries(self, entry_ids: list[int] | None = None, *, export_properties: list[str] | None = None, export_type: Literal[ExportType.text] = ExportType.text, json_dump: bool = False, json_indent: int = 4) -> str:
		...
	@overload
	def export_entries(self, entry_ids: list[int] | None = None, *, export_properties: list[str] | None = None, export_type: Literal[ExportType.json] = ExportType.json, json_dump: bool = False, json_indent: int = 4) -> OrderedDict[str, Any] | list[OrderedDict[str, Any]]:
		...
	...
	def import_entries(self, data: str | list[dict[str, Any]], export_type: ExportType = ExportType.text) -> None:
		...
		try:
			for entry in data:
				...
				if entry_id < 0 or entry_id >= self.entry_count():
					raise PyMSError('Import', f"Invalid entry id (must be between 0 and {self.entry_count()}, got {entry_id})")
