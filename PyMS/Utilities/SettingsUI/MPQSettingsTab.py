...
class MPQSettingsTab(SettingsTab):
	def __init__(self, *, parent: UI.Notebook, edited_state: EditedState, mpq_hander: MPQHandler, mpqs_config: Config.List[str], mpqs_select_config: Config.SelectFile) -> None:
		...
		UI.Label(self, text='MPQ Settings:', font=UI.Font.default().bolded(), anchor=UI.W).pack(fill=UI.X)
		UI.Label(self, text="Files will be read from the highest priority MPQ that contains them.\nThe higher an MPQ is on the list the higher its priority.", anchor=UI.W, justify=UI.LEFT).pack(fill=UI.X)
		...
		for mpq in self.mpqs_config.data:
			self.listbox.insert(UI.END,mpq)
		if self.listbox.size():
			self.listbox.select_set(0)

		self.toolbar = UI.Toolbar(self)
		self.toolbar.add_button(Assets.get_image('add'), self.add, 'Add MPQ', UI.Key.Insert)
		self.toolbar.add_button(Assets.get_image('remove'), self.remove, 'Remove MPQ', UI.Key.Delete, enabled=False, tags='has_selection')
		self.toolbar.add_button(Assets.get_image('opendefault'), self.adddefault, "Add default StarCraft MPQ's", UI.Shift.Insert)
		self.toolbar.add_spacer(10, flexible=True)
		def move_callback(direction: int) -> Callable[[], None]:
			def move() -> None:
				self.movempq(direction=direction)
			return move
		self.toolbar.add_button(Assets.get_image('up'), move_callback(0), 'Move MPQ Up', UI.Shift.Up, enabled=False, tags='has_selection')
		self.toolbar.add_button(Assets.get_image('down'), move_callback(1), 'Move MPQ Down', UI.Shift.Down, enabled=False, tags='has_selection')
	...
	def movempq(self, _event: UI.Event | None = None, direction: int = 0) -> None:
	...
	def add(self, _event: UI.Event | None = None, add: list[str] | None = None) -> None:
		n: str | int = 0
		s: int = 0
		if add is None:
