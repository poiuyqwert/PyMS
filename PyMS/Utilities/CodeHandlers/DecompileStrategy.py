...
class DecompileStrategyBuilder:
	...
	def _add_block(self, block: CodeBlock, suffix: str | None = None) -> None:
		...
		# Walk back to the earliest block not yet added (was recursive on prev_block).
		if not block in self.items:
			while block.prev_block and not block.prev_block in self.items:
				block = block.prev_block
		# Add blocks forward along the next_block chain iteratively so a long
		# linear chain doesn't exhaust the recursion limit on large scripts.
		current: CodeBlock | None = block
		while current is not None:
			if current in self.items:
				if self.active_header is not None and (block_headers := self.block_headers.get(current)) and not self.active_header in block_headers:
			...
			if self.active_header is not None:
				block_headers.append(self.active_header)
			self.block_headers[current] = block_headers
			for ref_block in current.ref_blocks:
	...
	def build(self) -> DecompileStrategy:
		while self._external_headers:
			self.add_header(self._external_headers.pop(0))
