...
class Canvas(_Tk.Canvas, Extensions):
	class Item(object):
		def __init__(self, canvas, item_id):
			self.canvas = canvas # type: _Tk.Canvas
	...
	def create_line(self, *points, **kwargs):
		...
		return Canvas.Item(self, _Tk.Canvas.create_line(self, *points, **kwargs))
	...
	def create_polygon(self, *points, **kwargs):
		points = tuple(self.coordinate_adjust(points[n],points[n+1]) for n in range(0,len(points),2))
		return Canvas.Item(self, _Tk.Canvas.create_polygon(self, *points, **kwargs))
